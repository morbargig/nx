...
export type tableElStyleObj = { styleClass?: string, style?: { [k: string]: string }, styleClassObj?: { [k: string]: boolean } }
type tableElementsStyleObj<T extends string = ''> = { [key in T]?: tableElStyleObj }
...
export class TableComponent<T = any, E = T> {
  ...
  public getElStyleObj = (el: tableElements): tableElStyleObj => [this.defaultTableBodyStyle?.[el] || {},
    this.tableBodyStyle?.[el] || {}]?.reduce((p, c) => ({
    style: {...(p?.style || {}), ...(c?.style || {})},
    styleClass: (p?.styleClass || '') + ' ' + (c?.styleClass || ''),
    styleClassObj: {...(p?.styleClassObj || {}), ...(c?.styleClassObj || {})}
  }), {})
  public getElStyleClassString = (el: tableElements, inputStyleClassObj?: tableElStyleObj['styleClass']): tableElStyleObj['styleClass'] => (inputStyleClassObj || '') + ' ' + (this.getElStyleObj(el)?.styleClass || '')
  public getElStyleClassObj = (el: tableElements): tableElStyleObj['styleClassObj'] => this.getElStyleObj(el)?.styleClassObj
  public getElStyleClass = (el: tableElements, inputStyleClassObj?: tableElStyleObj['styleClass'],
                            ngClassObj?: tableElStyleObj['styleClassObj']): tableElStyleObj['styleClassObj'] =>
    ({[this.getElStyleClassString(el, inputStyleClassObj) || '']: true, ...this.getElStyleClassObj(el), ...ngClassObj})
  public getElStyle = (el: tableElements, inputStyleObj?: tableElStyleObj['style']): tableElStyleObj['style'] => ({...(this.getElStyleObj(el)?.style || {}), ...(inputStyleObj || {})})
  ...
  @Input()
  public set columns(cols: ITableColumn<T>[]) {
    this._columns = cols
  }
  ...
  getExtendsColSpan = (lastTd: boolean) => (this.columns?.length / (this?.extendsColumns?.length || 0) +
  ...
  getExtendsDataItems = (items: T[] | undefined = this.items): E[] | undefined => items?.map(
    i => this.getExtendsData!?.(i))
  panelOpenState: boolean[] = [];
  togglePanel = (index: number, state?: boolean) => {
    if (!!this.extendsColumns?.length || !!this.getExtendsColumnsArr) {
      this.panelOpenState[index] = (state !== undefined) ? (state) : (!this.panelOpenState[index])
    }
  }
  ...
  @Input() showMoreTextsObj?: { more: string, less: string } = {more: "כל הפריטים", less: 'הסתר'}
  moreItems = (): void => {
  ...
  @ContentChild('colTempRef', {static: false}) public colTempRef?: TemplateRef<{
    item: T,
    col: ITableColumn<T>,
    $implicit: T[keyof T]
  }>
