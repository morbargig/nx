...
class TableCellClassType<T extends any = any, K extends keyof T = keyof T> {
  TableCellDicType = asConst<{ [key in keyof typeof TableColumnType]+?: Type<BaseCellComponent<any, any>> }>()({
...
type TableCellComponentType<T extends any = any, K extends keyof T = keyof T> = (TableCellClassType<T, K>['TableCellDicType'])
...
export interface BaseCellData<T = any, K extends keyof T = keyof T /** will have used on extends components */> {
}

export interface ITableCell<T = any,
...
interface ITableColumnObj<T = any,
  DModel = any,
   ...
   */
  parsedHtmlData?: (val?: T[K], item?: T, parseData?: T[K] | T | any) => Component['template'];
  styleClass?: string;
  ...
  colspan?: number;
  hidden?: boolean;
  hiddenFunc?: (data: { val?: T[K], item?: T, isHeader?: boolean, items: T[] }) => boolean;
  headerStyleFunc?: (((this: ITableColumnObj<T
      // , DModel, K, E
      >) => tableElStyleObj)
    |
    (() => tableElStyleObj)
...
export type ITableColumn<T extends {} = any> = {
