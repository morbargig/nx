...
export class FormArrayData<T = any, K extends keyof T = keyof T> implements BaseFieldData<T, K> {
  public formGroupConfig?: K extends '_' ? never : (T[K] extends (infer U)[] ?
    (U extends object ? (U extends any[] ? never : DynamicFormControlArray<U>) : never)
    : never);
  // public formControlConfig?: T[K] extends (infer U)[] ? (U extends JsonPrimitive ? DynamicFormControl<{ '_': U }> : never) : never;
  public formControlConfig?: (T[K] extends (infer U)[] ? (U extends object ?
      never :
  ...
  public formArrayConfig?: K extends '_' ? never : (T[K] extends (infer U)[] ? (U extends any[] ?
    (FieldConfigObj<{ '_': U }, FormArrayData<{ '_': U }>, keyof { '_': U }, 'FormArray', FormArray>) : never) : never);
  public groupValidations?: ValidatorFn[] = [];
  public asyncGroupValidations?: AsyncValidatorFn[] = [];

  constructor(obj?: FormArrayData<T, K>) {
    (Object.keys(obj || {}) as (keyof FormArrayData<T, K>)[])?.forEach((key) => ((<any>this)[key] = obj[key]));
  }
